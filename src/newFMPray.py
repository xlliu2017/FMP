...
class _FrameletAggregatorLayer(nn.Module):
    ...
    def forward(self, x_in, subband_ops):
        """
        Steps:
          1) if use_input_linear, x_in => lin_in => x_in_l => [N, out_dim]
          2) sum_{i}( alpha_i * spmm(subband_ops[i], x_in_l ) )
          3) BN => ReLU => Dropout
        """
...
class ThreeBranchFramelet(nn.Module):
    ...
    def __init__(self,
        ...
        # first aggregator layer from in_dim->poly_hidden_dim
        ...
        self.agg_attn = ThreeBranchAttnAggregator(d_in=final_hidden_dim, d_out=final_hidden_dim, n_heads=aggregator_heads)
    ...
    def forward(self, x, adjacency_frame, adjacency_linkx):
        ...
        for layer in self.agg_layers:
            x_poly = layer(x_poly, self.d_list)
...
###############################################################################
dataset_name = "chameleon"  # example
...
def train_with_tune(config):
    """
    This function is called by Ray Tune with hyperparameters in 'config'.
    We'll:
      1) Load chameleon dataset
      2) Possibly rewire
      3) Build signless Laplacian, wavelet ops
      4) Create ThreeBranchFramelet model with hyperparams from 'config'
      5) Run multiple reps (say 10 times), each time training from scratch 
         to measure variance across seeds or runs
      6) tune.report({"test_acc_mean": ..., "test_acc_var": ...})

    config keys:
      - Lev, poly_degree, n_framelet_layers, poly_hidden_dim, local_hidden_dim, linkx_embed_dim
      - aggregator_heads, final_hidden_dim, dropout, lr, weight_decay, epochs
      - rewire_flag
    """
    ...
    # We'll do 10 repeated runs to collect variance
    n_reps = 10
    all_test = []

    for rep in range(n_reps):
        # Build a new model each rep
...
def run_ray_tune():
    ...
    search_space = {
        ...
        "FrameType":          tune.choice(["Haar","Linear", "Quadratic"])
    }
    ...
    tuner = tune.run(
        ...
        metric="test_acc_mean",   # or "test_acc_var" if you want
        mode="max"
    )
    ...
    # You can do more analysis on df
